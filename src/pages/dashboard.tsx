...
import { getBirthcharts } from 'apiSdk/birthcharts';
import { BirthchartInterface } from 'interfaces/birthchart';
import { getReadings } from 'apiSdk/readings';
import { ReadingInterface } from 'interfaces/reading';

import { ReadingListPage as ShowCaseList } from 'pages/readings';

function DashboardPage() {
  const { session } = useSession();

  const chatStat = useSWR<PaginatedInterface<ChatInterface>>(
    () => `/chats`,
    () => getChats({ limit: 1, offset: 0 }),
  );

  const birthchartStat = useSWR<PaginatedInterface<BirthchartInterface>>(
    () => `/birthcharts`,
    () => getBirthcharts({ limit: 1, offset: 0 }),
  );

  const readingStat = useSWR<PaginatedInterface<ReadingInterface>>(
    () => `/readings`,
    () => getReadings({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Chats',
      result: chatStat?.data?.totalCount,
      isLoading: chatStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Birthcharts',
      result: birthchartStat?.data?.totalCount,
      isLoading: birthchartStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Readings',
      result: readingStat?.data?.totalCount,
      isLoading: readingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`readings`} passHref legacyBehavior>
                  <Button
